...
import { StyledUserReportsList, TextNoReport } from "./style";
  ...
  return (
    <StyledUserReportsList>
      {userReports.length > 0 ? (
        userReports.map((report) => <ReportCard key={report.id} report={report} />)
      ) : (
        <TextNoReport>Não há reports feitos.</TextNoReport>
      )}
    </StyledUserReportsList>
  );
};
